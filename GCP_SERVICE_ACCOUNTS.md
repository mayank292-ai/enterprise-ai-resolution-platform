...
The default compute service account in your project has been de-privileged. Whenever you provision compute (e.g., a VM, Cloud Run service, or Cloud Function), you **must** attach your Workload SA: `workload@hack-team-finsightai.iam.gserviceaccount.com`.

This is usually referred to in the GCP documentation as "attaching a custom SA"

Both your [GitHub Actions workflows](./.github/workflows/) and [Terraform Cloud workspace](https://app.terraform.io/app/db-hackathon-2026/workspaces/hack-team-finsightai) have pre-populated variables containing the Workload SA email.
...
*   **gcloud:** [Configure user-specified service accounts](https://cloud.google.com/build/docs/securing-builds/configure-user-specified-service-accounts)
    *   Ensure you specify the `--service-account=workload@hack-team-finsightai.iam.gserviceaccount.com` parameter of `gcloud builds submit`.
...
```shell
PROJECT_ID="hack-team-finsightai"

curl \
-X POST \
-H "Authorization: Bearer $(gcloud auth print-access-token)" \
-H "Content-Type: application/json; charset=utf-8" \
https://europe-west4-aiplatform.googleapis.com/v1/projects/hack-team-finsightai/locations/europe-west4/pipelineJobs?pipelineJobId=tune-large-model-$(date +"%Y%m%d%H%M%S") -d \
$'{
    "displayName": "tune-large-model",
    "serviceAccount": "workload@hack-team-finsightai.iam.gserviceaccount.com",
    "runtimeConfig": {
        "gcsOutputDirectory": "gs://artifacts.hack-team-finsightai.appspot.com/Foo/",
        "parameterValues": {
            "location": "us-central1",
            "project": "hack-team-finsightai",
            "large_model_reference": "text-bison@001",
            "model_display_name": "mytunedmodel_1",
            "train_steps": 100,
            "encryption_spec_key_name": "",
            "dataset_uri": "gs://artifacts.hack-team-finsightai.appspot.com/Bar.jsonl",
            "evaluation_data_uri": "",
...
Gemini runs on the Enterprise Agent Platform, the service formerly called Vertex AI. The `aiplatform.googleapis.com` API is enabled on your project and your Workload SA (`workload@hack-team-finsightai.iam.gserviceaccount.com`) has `roles/aiplatform.user`, which is enough to send prompts to Gemini. Because you cannot create service account keys, authenticate through Application Default Credentials (ADC) in both cases below.
...
gcloud auth application-default set-quota-project hack-team-finsightai
```
...
export GOOGLE_CLOUD_PROJECT="hack-team-finsightai"
