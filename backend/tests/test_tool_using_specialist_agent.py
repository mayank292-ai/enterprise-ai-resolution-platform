...
@pytest.mark.asyncio
async def test_retries_incomplete_finish_until_result_is_present() -> None:
    """Recover when the model repeatedly omits FINISH.final_result."""

    expected_result = create_specialist_result(
        evidence=create_evidence(),
        confidence=0.9,
    )
    incomplete_finish = SpecialistStep(
        step_type=SpecialistStepType.FINISH,
        reasoning="The investigation is complete.",
    )
    model_client = FakeModelClient(
        [
            incomplete_finish.model_copy(deep=True),
            incomplete_finish.model_copy(deep=True),
            SpecialistStep(
                step_type=SpecialistStepType.FINISH,
                reasoning="The trusted evidence supports the conclusion.",
                final_result=expected_result,
            ),
        ]
    )
    tool_catalog = Mock()
    tool_catalog.build_model_catalog.return_value = "No tools required."
    tool_catalog.execute = AsyncMock()
    agent = DummySpecialistAgent(
        model_client=model_client,
        tool_catalog=tool_catalog,
    ...
    result = await agent.investigate(
        investigation=create_investigation(),
        objective="Summarize the verified evidence.",
        evidence_needed=["A factual conclusion"],
    )
...
async def test_stops_after_maximum_tool_steps() -> None:
    ...
    tool_catalog.execute.assert_awaited_once()
